...
class RoleResource extends Resource
{
                        ...
                        Section::make()
                            ->schema([
                                TextInput::make('name')
                                    ...
                                    ->required()
                                    ->maxLength(255)
                                    // This stops anyone from editing the role name string
                                    ->disabled(fn ($record): bool => $record && in_array($record->name, self::$protectedRoles)),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            // Select record for bulk delete
            ->checkIfRecordIsSelectableUsing(
                fn ($record): bool => ! in_array($record?->name, self::$protectedRoles),
            )
            ->recordActions([
                EditAction::make(),
    ...
    public static function canEdit(Model $record): bool
    {
        ...
        if (in_array($record->name, self::$protectedRoles, true)) {
            return false;
        }
